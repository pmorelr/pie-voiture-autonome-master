...
def adaptevitesserelat (Mm1,M,MMMR,alpha,v,deltat,rmax):
    ...
    while i<nnn:
        theta=mmmr[i][0]
        theta=int(theta)
        print("M[theta][1]: ", M[theta][1])
        print("cos(theta*2*pi/360: ", cos(theta*2*pi/360))
        a = v*deltat/M[theta][1]/(1-cos(theta*2*pi/360)/max(sin(theta*2*pi/360),0.01))
        print("a antigo: ", a)
        if a > 1:
            a = 1
        if a < -1:
            a = -1
        print("a novo: ", a)
        vrel=(M[theta][1]-Mm1[int((acos(a))*360/(2*pi))][1])/deltat #approx de la derivee de la distance a l'objet par rapport au temps"   ATTENTION REMPLACER (orientation-orientationm1) par les donnees de l'accelerometre dorientation/dt*deltat
